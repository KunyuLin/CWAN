...
def collect_feature_and_target(data_loader: DataLoader, feature_extractor: nn.Module,
                                   device: torch.device, max_num_features=None) -> torch.Tensor:
    """
    Fetch data from `data_loader`, and then use `feature_extractor` to collect features
    ...
    with torch.no_grad():
        for i, (images, target) in enumerate(tqdm.tqdm(data_loader)):
            ...
            if max_num_features is not None and i >= max_num_features:
                break
    return torch.cat(all_features, dim=0).numpy(), torch.cat(all_targets, dim=0).numpy()
