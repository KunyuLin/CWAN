...
from dalib.partial_adaptation.dann import ImageClassifier
from dalib.partial_adaptation.cluster_src import ClusterModuleEuclidean, center_loss
...
def main(args: argparse.Namespace):
    ...
    clu_module = ClusterModuleEuclidean(num_cluster=num_classes, src_cluster=src_center, momentum=args.cluster_momentum)
    ...
    for epoch in range(args.epochs):
        # train for one epoch
        train(train_source_iter, classifier, clu_module, optimizer,
...
def train(train_source_iter: ForeverDataIterator, model: Classifier,
          clu_module: ClusterModuleEuclidean,  optimizer: SGD,
          lr_scheduler: LambdaLR, epoch: int, args: argparse.Namespace):
    batch_time = AverageMeter('Time', ':4.2f')
    data_time = AverageMeter('Data', ':3.6f')
    ...
    for i in range(args.iters_per_epoch):
        ...
        ctr_loss = center_loss(f_s, clu_module.src_cluster.clone().detach(), labels_s)
        loss = cls_loss + args.src_ctr_factor * ctr_loss
        ...
        # update clusters  
        clu_module.src_cluster_update(f_s.detach(), labels_s)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--src-ctr-factor', default=0.1, type=float)
    # training parameters
